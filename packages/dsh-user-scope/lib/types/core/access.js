...
import { emptyOwnershipSnapshot } from "./schema.js";
export { decideAccess };
...
export class UserScopeRegistry {
    ...
    addPrincipal(principal) {
        ...
        if (existing !== undefined) {
            if (existing.kind !== principal.kind)
                throw new Error('principal id collision');
            return;
        }
        this.state.principals.push(clonePrincipal(principal));
    }
    registerDevice(binding) {
        if (!this.state.principals.some(item => item.id === binding.principalId))
            throw new Error('device principal is not registered');
        const existing = this.state.devices.find(item => item.deviceId === binding.deviceId);
        if (existing !== undefined && existing.principalId !== binding.principalId)
    ...
    touchDevice(deviceId, now) {
        ...
        if (binding === undefined || binding.revokedAt !== undefined)
            return false;
    ...
    grantWorkspace(grant) {
        if (!this.state.principals.some(item => item.id === grant.principalId))
            throw new Error('grant principal is not registered');
    ...
    registerSession(ownership) {
        if (!this.state.principals.some(item => item.id === ownership.createdByPrincipalId))
            throw new Error('session principal is not registered');
        const existing = this.state.sessions.find(item => item.sessionId === ownership.sessionId);
        if (existing !== undefined && existing.createdByPrincipalId !== ownership.createdByPrincipalId)
            throw new Error('session ownership collision');
        if (existing === undefined)
            this.state.sessions.push(cloneSession(ownership));
        else
            Object.assign(existing, cloneSession(ownership));
    }
    grantSession(principalId, sessionId) {
        if (!this.state.principals.some(item => item.id === principalId))
            throw new Error('session grant principal is not registered');
        const session = this.state.sessions.find(item => item.sessionId === sessionId);
        if (session === undefined)
            throw new Error('session grant target is not registered');
        const grants = session.grantedPrincipalIds ?? [];
        if (!grants.includes(principalId))
            session.grantedPrincipalIds = [...grants, principalId];
    }
    revokeSession(principalId, sessionId) {
        const session = this.state.sessions.find(item => item.sessionId === sessionId);
        if (session === undefined || session.grantedPrincipalIds === undefined)
            return false;
        const next = session.grantedPrincipalIds.filter(item => item !== principalId);
        if (next.length === session.grantedPrincipalIds.length)
            return false;
        if (next.length === 0)
            delete session.grantedPrincipalIds;
        else
            session.grantedPrincipalIds = next;
        return true;
    }
