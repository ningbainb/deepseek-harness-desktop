...
`.trim();
		...
		function findScrollRoot(pane, turns = getUserMessages(pane)) {
			const official = pane.matches("[data-conversation-scroll]") ? pane : pane.querySelector("[data-conversation-scroll]");
			if (official !== null) return official;
			const candidates = [pane, ...Array.from(pane.querySelectorAll("*"))].filter((child) => {
				const style = getComputedStyle(child);
				return style.overflowY === "auto" || style.overflowY === "scroll";
			});
			const containingAllTurns = turns.length === 0 ? candidates : candidates.filter((candidate) => turns.every((turn) => candidate.contains(turn)));
			...
			for (const child of pool) if (!pool.some((other) => other !== child && child.contains(other))) return child;
			return pane;
		}
		function viewportTop(scrollRoot) {
		...
		function currentTurnIndex(scrollRoot, turns) {
			...
			for (let i = turns.length - 1; i >= 0; i--) if ((turns[i]?.getBoundingClientRect().top ?? Number.POSITIVE_INFINITY) <= viewAnchor) return i;
			return 0;
		}
		...
		function bindNavigator(nav, scrollRoot, pane) {
			nav.addEventListener("click", (e) => {
				...
				if (role === "prev" && idx > 0 && turns[idx - 1]) scrollToTurn(root, turns[idx - 1]);
				else if (role === "next" && idx < turns.length - 1 && turns[idx + 1]) scrollToTurn(root, turns[idx + 1]);
				else if (role === "bottom") root.scrollTo({
