...
import { isCompleteModelRoute, isEffectivelyActive, resolveEffectiveConfig, resolveResolvedConfig, resolveSessionConfig } from './config.ts'
import { buildExpertSystemPrompt } from './policy.ts'
import { valueModeState } from './state.ts'

type DefaultModelService = {
  currentSelection?: () => ModelRouteSelection
}

function readDefaultExpert(ctx: Context): ModelRouteSelection | undefined {
...
export function consultExpertCallView(args: ConsultExpertArgs): GenericCallView {
...
export function parseExpertResponse(rawText: string, purpose: string): Omit<ConsultExpertOutput, 'success' | 'purpose' | 'model'> {
...
export function createConsultExpertTool(
  ctx: Context,
  getConfig: () => ValueModeConfig,
): ToolDefinition {
  return defineTool({
    name: 'consult_expert',
    description:
      'Legacy compatibility tool for integrations that still request a bounded expert consultation. ' +
    ...
    output: {
      ...
      render: (_args, value) => {
        const val = value as unknown as ConsultExpertOutput
    ...
    async execute(rawArgs, exec) {
      const args = rawArgs as ConsultExpertArgs
      const globalConfig = getConfig()
      const sessionId = (exec as unknown as { sessionId?: string })?.sessionId || 'default'
