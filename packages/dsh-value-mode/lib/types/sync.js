...
import { validateAgentCordis } from "./preset-schema.js";
/**
...
function sameFile(a, b) {
    ...
    if (sourceStat.size !== targetStat.size)
        return false;
    if (Math.abs(sourceStat.mtimeMs - targetStat.mtimeMs) > MTIME_TOLERANCE_MS)
        return false;
...
function validatePresetAgentFile(presetDir) {
    ...
    if (!existsSync(agent))
        return ['agent.cordis.yml is missing from the preset tree'];
...
export function syncPresetTrees(sourceRoot, targetRoot, retire = []) {
    ...
    for (const id of retire) {
        if (existsSync(join(sourceRoot, id)))
            continue;
