...
用量中心明确区分三类指标。只有 `deepseek-official` 具备已验证的
`/user/balance` 接口且返回完整余额记录时，才显示官方账户余额；不支持的 Provider、
缺失明细或请求失败均显示未知，不会用零补齐。`≈` 费用是按所选峰谷时段计算的
本地估算，不是账户余额或账单。TPS 是流式输出的滚动 1 秒峰值，不是计费速率。
