...
describe('Desktop Client SDK v1', () => {
  ...
  it('uses a narrow typed contract without exposing the preload bridge', async () => {
    ...
    expect(await client.openDesktopSurface('extensions', { setting: 'models' })).toBe(true)
    expect(await client.openDesktopSurface('extensions', { tab: 'plugins' })).toBe(true)
    expect(await client.openDesktopSurface('extensions', { tab: 'market' })).toBe(true)
    expect(await client.openDesktopSurface('extensions', { tab: 'skills' })).toBe(true)
    ...
    expect(bridge.openExtensionDock).toHaveBeenNthCalledWith(3, { setting: 'models' })
    expect(bridge.openExtensionDock).toHaveBeenNthCalledWith(4, { tab: 'plugins' })
    expect(bridge.openExtensionDock).toHaveBeenNthCalledWith(5, { tab: 'market' })
    expect(bridge.openExtensionDock).toHaveBeenNthCalledWith(6, { tab: 'skills' })
    ...
    await expect(client.openDesktopSurface('extensions', { tab: 'settings' as never })).rejects.toMatchObject({ code: 'desktop-invalid-argument' })
