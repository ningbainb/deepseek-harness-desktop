...
export const DEFAULT_PERSONAL_PROMPT = {
    version: 1,
    enabled: false,
    profiles: [],
};
...
function parseProfile(value, strict) {
    if (!isRecord(value)) {
        if (strict)
            throw new InvalidPersonalPromptError('profile must be an object');
        return undefined;
    }
    const id = value.id;
    ...
    if (!safeId(id) || !promptName(name) || !promptContent(content) || typeof enabled !== 'boolean' || !profileScope(scope) || !validTime(value.updatedAt)) {
        if (strict)
            throw new InvalidPersonalPromptError('profile contains an invalid field');
        return undefined;
    }
    if (scope === 'global' && (workspaceId !== undefined || sessionId !== undefined)) {
        if (strict)
            throw new InvalidPersonalPromptError('global profile cannot carry workspaceId or sessionId');
        return undefined;
    }
    if (scope === 'workspace' && (!safeId(workspaceId) || sessionId !== undefined)) {
        if (strict)
            throw new InvalidPersonalPromptError('workspace profile requires workspaceId');
        return undefined;
    }
    if (scope === 'session' && (!safeId(sessionId) || workspaceId !== undefined)) {
        if (strict)
            throw new InvalidPersonalPromptError('session profile requires sessionId');
        return undefined;
    }
    return {
        id,
...
export function normalizePersonalPrompt(value) {
    if (value === undefined)
        return { ...DEFAULT_PERSONAL_PROMPT, profiles: [] };
    if (!isRecord(value))
        throw new InvalidPersonalPromptError('personal-prompt must be an object');
    if (value.version !== undefined && value.version !== 1)
        throw new InvalidPersonalPromptError('unsupported personal-prompt version');
    const profiles = [];
    if (Array.isArray(value.profiles)) {
        for (const candidate of value.profiles) {
            const profile = parseProfile(candidate, false);
            if (profile === undefined || profiles.some(item => item.id === profile.id) || profiles.length >= MAX_PROMPT_PROFILES)
                continue;
...
export function assertPersonalPrompt(value) {
    ...
    for (const candidate of value.profiles) {
        ...
        if (ids.has(profile.id))
            throw new InvalidPersonalPromptError('personal-prompt profile IDs must be unique');
        ids.add(profile.id);
    }
...
export function upsertPromptProfile(config, profile) {
    ...
    if (index < 0) {
        if (profiles.length >= MAX_PROMPT_PROFILES)
            throw new InvalidPersonalPromptError('too many personal-prompt profiles');
...
export function removePromptProfile(config, profileId) {
    const normalized = normalizePersonalPrompt(config);
...
export function setActivePromptProfile(config, profileId) {
    const normalized = normalizePersonalPrompt(config);
...
function matchingProfile(profile, context) {
    if (!profile.enabled || profile.content.length === 0)
        return false;
    if (profile.scope === 'global')
        return true;
    if (profile.scope === 'workspace')
        return context.workspaceId !== undefined && profile.workspaceId === context.workspaceId;
...
function pickProfile(profiles, activeProfileId) {
    ...
    return profiles.reduce((selected, profile) => {
        ...
        if (selected === undefined
            ...
            || (profile.updatedAt === selected.updatedAt && profile.id < selected.id))
            return profile;
        return selected;
    }, undefined);
}
/** Resolve exactly one profile with the fixed session > workspace > global precedence. */
export function resolveEffectivePrompt(config, context = {}) {
    const normalized = normalizePersonalPrompt(config);
...
export function renderPromptProfile(profile) {
    if (profile === undefined || profile.content.length === 0)
        return '';
...
export function promptVariableValue(profile) {
    if (profile === undefined || profile.content.length === 0)
        return '';
    const budget = Math.max(0, MAX_ASSEMBLED_PROMPT_LENGTH - PROMPT_PREFIX.length - PROMPT_SUFFIX.length);
