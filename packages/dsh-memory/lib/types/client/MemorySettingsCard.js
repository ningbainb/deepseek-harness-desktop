...
export function MemorySettingsCard(props) {
    ...
    return (_jsxs("section", { className: styles.card, "data-memory-card": "true", "data-dock-owner": "memory", "data-dock-dirty": editorOpen && JSON.stringify(draft) !== cleanDraft, children: [_jsxs("header", { className: styles.header, children: [_jsxs("div", { children: [_jsx("h3", { className: styles.title, children: t('settings.title') }), _jsx("p", { className: styles.description, children: t('settings.description') })] }), _jsx("span", { className: saved ? styles.badgeSaved : styles.badge, children: saving ? t('settings.saving') : saved ? t('settings.saved') : editorOpen && JSON.stringify(draft) !== cleanDraft ? t('settings.unsaved') : t('settings.ready') })] }), _jsx("p", { className: styles.notice, children: t('settings.ownerNotice') }), _jsx(MemoryActivityPanel, { t: t, enabled: enabled, onEdit: select }), saved && selectedId && _jsxs("p", { className: styles.muted, role: "status", children: [t('settings.savedTo'), " ", t(('settings.' + draft.scope)), draft.workspaceId || draft.sessionId ? ' · ' + (draft.workspaceId || draft.sessionId) : ''] }), !settingsSnapshot.writable && _jsx("p", { className: styles.notice, role: "status", children: t('settings.readonly') }), settingsSnapshot.status === 'loading' && _jsx("p", { className: styles.muted, role: "status", children: t('settings.loading') }), error && _jsx("p", { className: styles.error, role: "alert", children: error }), _jsxs("label", { className: styles.toggle, children: [_jsx("input", { type: "checkbox", checked: enabled, disabled: !settingsSnapshot.writable || saving, onChange: event => saveConfig(event.target.checked) }), t('settings.enabled')] }), dock && _jsxs("div", { className: styles.tabs, role: "tablist", "aria-label": t('settings.title'), onKeyDown: event => {
