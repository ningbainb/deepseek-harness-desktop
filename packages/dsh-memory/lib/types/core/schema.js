...
export function isSafeMemoryId(value) {
...
export function containsSensitiveMemoryContent(content) {
    return SENSITIVE_PATTERNS.some(pattern => pattern.test(content));
}
function assertMemoryContent(content) {
    if (!isContent(content))
        throw new MemoryValidationError('memory content is empty or too long');
    if (containsSensitiveMemoryContent(content))
        throw new MemoryValidationError(MEMORY_SENSITIVE_MESSAGE, 'sensitive');
}
function parseItem(value, strict, expectedPrincipalId) {
    if (!isRecord(value)) {
        if (strict)
            throw new MemoryValidationError('memory item must be an object');
        return undefined;
    }
    const id = value.id;
    ...
    const invalid = () => {
        if (strict)
            throw new MemoryValidationError('memory item contains an invalid field');
        return undefined;
    };
    if (!isSafeMemoryId(id) || !isOpaqueId(principalId) || !isMemoryScope(scope) || !isContent(content) || !Array.isArray(tags) || tags.length > MAX_MEMORY_TAGS || !tags.every(isTag) || new Set(tags).size !== tags.length || typeof pinned !== 'boolean' || !isMemorySource(source) || !isTime(createdAt) || !isTime(updatedAt) || (expiresAt !== undefined && !isTime(expiresAt)))
        return invalid();
    if (containsSensitiveMemoryContent(content)) {
        if (strict)
            throw new MemoryValidationError(MEMORY_SENSITIVE_MESSAGE, 'sensitive');
    ...
    if (expectedPrincipalId !== undefined && principalId !== expectedPrincipalId)
        return invalid();
    if (scope === 'global' && (workspaceId !== undefined || sessionId !== undefined))
        return invalid();
    if (scope === 'workspace' && (!isOpaqueId(workspaceId) || sessionId !== undefined))
        return invalid();
    if (scope === 'session' && (!isOpaqueId(sessionId) || workspaceId !== undefined))
        return invalid();
    return {
        id,
...
export function normalizeMemorySnapshot(value, expectedPrincipalId) {
    if (value === undefined)
    ...
    for (const candidate of value.items) {
        ...
        if (item === undefined || items.some(existing => existing.id === item.id) || items.length >= MAX_MEMORY_ITEMS)
            continue;
        items.push(item);
    }
    return { version: MEMORY_SCHEMA_VERSION, items };
}
export function assertMemoryItem(value, expectedPrincipalId) {
    const item = parseItem(value, true, expectedPrincipalId);
    if (item === undefined)
        throw new MemoryValidationError('memory item is invalid');
}
export function assertMemorySnapshot(value, expectedPrincipalId) {
    ...
    for (const candidate of value.items) {
        ...
        if (ids.has(candidate.id))
            throw new MemoryValidationError('memory item IDs must be unique');
...
export function toPublicMemoryItem(item) {
    return {
        id: item.id,
        scope: item.scope,
