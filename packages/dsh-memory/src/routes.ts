...
export function isTrustedMemoryRequest(request: IncomingMessage): boolean {
...
function localContext(
  ...
  url: URL,
): ReturnType<MemoryService['localContext']> {
...
function draftFromBody(body: Record<string, unknown>): MemoryDraft | undefined {
  const scope = body.scope
  const content = body.content
  if (!isMemoryScope(scope) || typeof content !== 'string') return undefined
  const tags = body.tags
  if (tags !== undefined && (!Array.isArray(tags) || tags.some(tag => typeof tag !== 'string'))) return undefined
