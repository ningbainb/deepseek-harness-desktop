...
- **拖拽文件到输入框**：内部文件树行插入工作区相对路径；本机会话中的外部普通文件通过串行上传生成可移除的附件卡片，显示添加中、已添加或失败，支持重试，单文件上限 100 MB。文件保存在当前会话工作区的 `.dsh-attachments/` 下，发送时提供真实引用，不预先解析 PDF、Office 或二进制。图片继续使用原生附件与大图校验、压缩流程。没有文件队列接口的旧环境保留文本插入与路径引用回退。
...
附件上传仅接受同源回环请求与已存在的直接会话，从宿主会话头取得工作目录并核对工作区登记，拒绝调用方指定任意目录、目录穿越与外部符号链接。上传流有 100 MB 上限，使用独占新文件，失败清理未完成副本；不执行文件内容。移除卡片只移除草稿引用，不删除源文件或已上传副本，以保留历史引用。上传副本保存在工作区内，可能出现在文件树和 Git 未跟踪列表；不自动修改 Git 忽略规则。远程未配对访问继续被拒绝。
