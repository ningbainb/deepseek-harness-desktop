...
- **拖拽文件到输入框**：内部文件树行只会在草稿光标处插入工作区相对路径；这是发送后供 agent 读取的引用，不是预览或预先解析文档。小于 1 MB 的外部文本或代码文件会插入路径与围栏文本，PDF、Word、电子表格、压缩包、二进制及更大的文件只插入路径链接。外部超大图片按单文件依次经过校验、尺寸检查、解码、压缩和附件提交；按 Esc 可取消，失败后无需刷新页面即可重新拖入重试。
