...
it('gives an available native sidebar the initial layout without opening tabs or rewriting saved tools', () => {
  ...
  try {
    ...
    expect(shell.hasAttribute('data-aionui-instant')).toBe(true)
...
it('yields explorer space to the native dock and restores it without changing preferences', async () => {
  ...
  try {
    ...
    expect((document.querySelector('.aionui-floating-expand') as HTMLElement).style.display).toBe('flex')
    shell.style.gridTemplateColumns = '280px minmax(0, 1fr) 0px'
    ...
    layout.toggleExplorer()
    expect(activate).toHaveBeenCalledOnce()
    await vi.waitFor(() => expect(shell.querySelector<HTMLElement>('[data-aionui-explorer-col]')!.dataset.aionuiVisible).toBe('true'))
    expect(store.getSnapshot().explorerCollapsed).toBe(false)
    expect(shell.style.gridTemplateColumns).toMatch(/0px 0px 260px$/u)
