...
const temporary = await mkdtemp(join(tmpdir(), 'dsh-3-3-mode-flow-'))
...
try {
  ...
  await picker.locator('input[aria-label="编辑路径"]').press('Enter')
  await picker.getByRole('button', { name: '打开', exact: true }).click({ force: true })
  await picker.waitFor({ state: 'hidden', timeout: 15_000 })
  await page.waitForTimeout(4_000)
  const createButtons = page.getByRole('button', { name: '新建会话' })
  console.log(`new session buttons after workspace: ${await createButtons.count()}`)
  await createButtons.last().click({ force: true })
  await page.waitForTimeout(12_000)
  console.log(`after new session: ${JSON.stringify(await page.evaluate(() => ({
    text: document.body.innerText.replace(/\\s+/gu, ' ').slice(0, 2_500),
    modes: [...document.querySelectorAll('[data-dsh-mode-switcher="true"]')].map((node) => ({ text: node.textContent?.trim(), title: node.getAttribute('title'), rect: (() => { const r = node.getBoundingClientRect(); return { x: r.x, y: r.y, width: r.width, height: r.height } })() })),
  })), null, 2)}`)
  ...
  if (await mode.count() > 0 && await mode.isVisible().catch(() => false)) {
    ...
    console.log(`mode options: ${await page.getByRole('option').allTextContents()}`)
