...
- Desktop schema 4 adds a release analysis surface at `/admin/api/release?days=30&version=3.3.0`, protected by the existing admin session. It supports 7/30/90 days, optional exact version filtering, and aggregate CSV export in the dashboard. Daily release observations retain counts and stable installation presence by version, event, outcome and fixed detail for 90 UTC days. They contain no individual event timestamps or content. Schema 2/3 retain their original behavior and do not enter this table. Historical monthly observations are not backfilled.
- Release adoption is distinct installations with an observed launch in the selected version/window. An installation may appear in multiple versions; version totals are not additive. Startup success is accepted ready results divided by accepted ready plus failed results, with no-sample shown separately. Feature stages are independent observations, not an ordered conversion funnel. New signals cover project creation/connection, ordinary file attachment queue results, and Dock setting page opens; native image uploads and completed model tasks are outside these feature signals.
- Ingestion accepts at most 20 events and 16384 bytes per batch. The Desktop memory queue holds at most 200 waiting events, drains full batches and makes a bounded best-effort shutdown flush. Failed delivery is not persisted or retried, so counts are observational lower bounds.

- Active activity is an accepted schema 3/4 `app_launch` event. The client supplies an HMAC-derived `installationActor`; D1 stores only the 64-character pseudonymous value and a day presence row.
