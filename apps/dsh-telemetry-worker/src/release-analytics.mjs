...
export async function releaseSummary(db, filters, now = new Date()) {
  ...
  const [coverage, versions, events, active] = await Promise.all([
    ...
    db.prepare(`SELECT event, outcome, detail, SUM(count) AS count, COUNT(DISTINCT installation_actor) AS instances FROM product_release_daily WHERE ${where} GROUP BY event, outcome, detail ORDER BY count DESC, event, outcome, detail LIMIT 200`).bind(...bindings).all(),
