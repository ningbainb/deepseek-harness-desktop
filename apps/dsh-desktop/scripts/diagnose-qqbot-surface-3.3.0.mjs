...
try {
  ...
  const summary = await extension.evaluate(() => ({
    bodyText: document.body.innerText.slice(0, 5000),
    tabs: Array.from(document.querySelectorAll('[role="tab"], [data-tab]')).map(element => ({ text: element.textContent?.trim(), id: element.id, dataTab: element.getAttribute('data-tab'), selected: element.getAttribute('aria-selected') })),
    buttons: Array.from(document.querySelectorAll('button')).map(button => ({ text: button.textContent?.trim(), ariaLabel: button.getAttribute('aria-label'), id: button.id, data: Array.from(button.attributes).filter(attribute => attribute.name.startsWith('data-')).map(attribute => [attribute.name, attribute.value]) })).filter(button => /QQ|机器人|扫码|绑定|插件|native/iu.test(`${button.text} ${button.ariaLabel} ${button.id}`)),
    ids: Array.from(document.querySelectorAll('[id]')).map(element => element.id).filter(id => /qq|plugin|native/iu.test(id)),
  }))
  await extension.screenshot({ path: resolve(appDir, '..', '..', 'output', 'playwright', 'run-20260905', 'source-qqbot-surface.png') })
