...
try {
  ...
  const workspace = await main.evaluate(async path => {
    const response = await fetch('/api/workspace.create', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ type: 'client-request', rpcId: crypto.randomUUID(), method: 'workspace.create', payload: { path } }) })
  ...
  await main.evaluate(async workspaceId => {
    const response = await fetch('/api/session.create', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ type: 'client-request', rpcId: crypto.randomUUID(), method: 'session.create', payload: { workspaceId } }) })
    const value = await response.json()
