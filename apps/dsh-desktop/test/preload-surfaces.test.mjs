...
test('main preload exposes product actions without extension mutation channels', async () => {
  ...
  assert.match(source, /onPluginSettingsOpen: createBufferedSubscription\('desktop:plugin-settings-open'/u)
...
test('extension preload exposes extension operations without product update actions', async () => {
  ...
  assert.match(source, /openPluginSettings: \(name\) => ipcRenderer\.invoke\('extensions:plugin-settings-open', name\)/u)
...
test('Dock settings preload exposes only runtime transport and bounded settings actions', async () => {
  ...
  assert.match(source, /dock-settings:control-center-progress/u)
  assert.match(source, /onControlCenterProgress/u)
...
test('Electron binds each renderer window to its dedicated preload', async () => {
  ...
  assert.match(source, /updateChannel,\s*\r?\n\s*scheduleStore:\s*new DesktopUpdateCheckStore/u)
  assert.match(source, /isOnline:\s*\(\)\s*=>\s*electronNet\?\.isOnline/u)
  assert.match(source, /downloadRouter:\s*updateDownloadRouter/u)
})
