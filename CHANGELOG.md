...
- Agent Team、Browser Use、Computer Use 启停增加插件环境预检、重启进度与失败恢复提示；修正损坏插件环境时的交互和扩展管理入口。
- 修复打包版冷启动时性价比模式模型目录恰逢运行时或设置切换就提示“目录已变化”的问题；过期读取会有界自动重试，持续失败仍可手动重试。
- 主窗口位置与尺寸状态改为原子写入，避免事件驱动保存与启动读取并发时看到截断的 JSON。
- 自动更新检查加入离线延期与分类退避，手动检查继续可用；更新诊断按失败类型汇总，不采集用户路径、凭据或网络地址。
- 4.x 覆盖安装优先复用经过标记验证的现有安装目录，减少自定义目录用户升级后安装到其他位置的风险。
- 技能选择器按最近使用分组，插件设置从拓展坞直接进入，bai 模型组在选择器中显示推荐标识；保留 4.2.1 的登录、插件、历史会话和数据迁移保护。
- 内核继续固定 DSH `0.1.6-alpha.2`；上游 `0.1.7-alpha.1` 的 Session V4、Profile 设置及预设迁移未纳入本次版本。

## 4.2.1 - 2026-09-20

- 修复 ChatGPT 浏览器授权页显示成功、桌面端却未提交凭据的问题：浏览器模式一键启动，桌面端继续等待令牌交换、账户识别与本机凭据落盘，并提供设备码登录兜底和安全化失败分类。
- ChatGPT 授权等待改为低频、无重叠轮询；终态、取消和组件卸载都会停止后台读取，固定词表埋点不记录账号、令牌、URL、模型 ID 或原始错误。
- 更新下载窗口允许关闭，下载任务继续在后台存活；重新打开更新界面可继续查看进度和安装。
- 统一主界面插件、Skills 与模型管理入口，规整拓展坞相关页面和 bai 登录主路径；全平台应用、任务栏、托盘、安装器、官网与文档使用同一新版图标。
- 保留 4.2.0 的 DSH `0.1.6-alpha.2` 精确依赖、历史会话与升级数据保护；Windows 本地 Electron 回归和三端 GitHub Actions 构建继续作为发布门禁。
