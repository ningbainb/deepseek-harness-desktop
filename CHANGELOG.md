...
- 更新系统支持 `stable` 与 `beta`；Stable 拒绝 prerelease，Beta 不会改写 Stable 元数据，切回 Stable 不自动降级更高 Beta。发布产物新增 SHA-256 `release-manifest.json`，并在 `REQUIRE_SIGNING=true` 时强制证书、Authenticode signer/timestamp 验证；无证书的开发构建仍可明确保持未签名。
