...
DeepSeek Harness Desktop 3.0.9 keeps its public integration boundary deliberately small: the Desktop Contract and `@linxin666/dsh-desktop-client` SDK are the supported interfaces for plugins and desktop-aware web surfaces.
...
Desktop 3.0.9 does not block normal startup because a user plugin lacks publisher, registry, or compatibility metadata. The complete current profile is loaded directly. Compatibility declarations under `dsh.compatibility` remain useful diagnostic evidence in Extension Dock, but they are not a startup authorization list.
