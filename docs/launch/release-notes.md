# DeepSeek Harness Desktop 3.0.2
...
从同一 GitHub Release 下载 `DeepSeek-Harness-Desktop-Setup-3.0.2-x64.exe`、`SHA256SUMS.txt` 和 `release-manifest.json`。先比对 SHA-256，再查看 manifest 中的大小、频道、Runtime、Schema 与实际签名状态。未配置证书的社区 Release 可能是未签名版本，Windows 因此可能显示未知发布者提示。

### 说明

Desktop 默认不配置遥测上传端点，也不会自动上传诊断。自动修复只使用用户已经配置的模型；没有模型时直接进入同 Home 内置插件回退。3.0.2 不删除独立备份，也不承诺恢复 Desktop 之外的项目编辑或磁盘损坏。
