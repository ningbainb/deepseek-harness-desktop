...
const temporary = await mkdtemp(join(tmpdir(), 'dsh-3-3-mode-prompt2-'))
...
try {
  ...
  const visible = []
  for (let i = 0; i < count; i += 1) if (await textareas.nth(i).isVisible().catch(() => false)) visible.push(i)
  console.log(`visible textareas: ${JSON.stringify(visible)}`)
  if (visible.length === 0) throw new Error('no visible conversation textarea after workspace selection')
  const composer = textareas.nth(visible[visible.length - 1])
  console.log(`composer aria: ${await composer.getAttribute('aria-label')}, placeholder: ${await composer.getAttribute('placeholder')}`)
  ...
  console.log(`after prompt: ${JSON.stringify({ errors, modeCount: await page.locator('[data-dsh-mode-switcher="true"]').count(), body: (await page.locator('body').innerText()).replace(/\\s+/gu, ' ').slice(0, 3_000) }, null, 2)}`)
