...
const appDir = resolve(dirname(fileURLToPath(import.meta.url)), 'apps', 'dsh-desktop')
...
try {
  ...
  const inspection = await page.evaluate(() => {
    ...
    return {
      trigger: document.querySelector('button')?.outerHTML,
      visibleMenus: Array.from(document.querySelectorAll('[role="menu"], [role="listbox"], [data-radix-popper-content-wrapper], [data-state="open"]'))
        .filter(visible)
        .slice(0, 20)
        .map(element => ({ tag: element.tagName, role: element.getAttribute('role'), ariaLabel: element.getAttribute('aria-label'), text: element.textContent?.trim().slice(0, 500), html: element.outerHTML.slice(0, 1000) })),
      visibleButtons: Array.from(document.querySelectorAll('button'))
        .filter(visible)
        .map(button => ({ text: button.textContent?.trim(), ariaLabel: button.getAttribute('aria-label'), data: Array.from(button.attributes).filter(attribute => attribute.name.startsWith('data-')).map(attribute => [attribute.name, attribute.value]) }))
        .filter(button => /模型|Model|推理|reason|Codex|DeepSeek|选择/u.test(`${button.text} ${button.ariaLabel}`))
        .slice(-30),
      bodyText: document.body.innerText.slice(-3000),
    }
  })
  await page.screenshot({ path: resolve(process.cwd(), 'output', 'playwright', 'run-20260905', 'source-model-menu-diagnostic.png') })
