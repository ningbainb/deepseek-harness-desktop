...
const output = resolve(root, 'output', 'playwright', 'run-20260905', 'source-mode-ptc-selected-v2.png')
...
try {
  app = await electron.launch({ executablePath: electronPath, args: [resolve(appDir, 'src', 'main.mjs'), '--force-renderer-accessibility'], cwd: appDir, env: { ...process.env, DSH_DESKTOP_USER_DATA: userData, DSH_DESKTOP_DISABLE_UPDATES: '1', DSH_DESKTOP_VERIFY_UPDATER: '0', DSH_HOME: dshHome, DSH_AGENTS_HOME: join(userData, 'agents') } })
  ...
  for (let index = await dialogs.count() - 1; index >= 0; index -= 1) { const dialog = dialogs.nth(index); if (await dialog.isVisible().catch(() => false) && /内测声明|插件、技能和桌面核心功能在这里/u.test(await dialog.textContent().catch(() => ''))) { const button = dialog.getByRole('button', { name: /^(继续|Continue)$/u }).last(); if (await button.count()) await button.click({ force: true }) } }
  ...
  await textarea.fill('mode selection smoke')
  ...
  const selected = await page.locator('[data-dsh-mode-switcher="true"]').evaluate(element => ({ text: element.textContent?.trim(), buttons: Array.from(element.querySelectorAll('button')).map(button => button.textContent?.trim()) }))
  const bodyHasPtc = await page.locator('body').innerText().then(text => /PTC 模式/u.test(text))
  await page.screenshot({ path: output, animations: 'disabled', timeout: 60_000 })
  console.log(JSON.stringify({ options, selected, bodyHasPtc }, null, 2))
