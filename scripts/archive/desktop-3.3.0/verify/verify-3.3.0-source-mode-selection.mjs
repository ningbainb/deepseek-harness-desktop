...
const output = resolve(root, 'output', 'playwright', 'run-20260905', 'source-mode-ptc-selected.png')
...
try {
  ...
  await textareas.nth(visible.at(-1)).fill('mode selection smoke')
  ...
  const selected = await mode.getByRole('button').textContent()
  const attr = await mode.getAttribute('data-dsh-mode-switcher')
  await page.screenshot({ path: output, animations: 'disabled', timeout: 60_000 })
  console.log(JSON.stringify({ options, selected: selected?.trim(), attr, errors }, null, 2))
} finally {
  await app?.close()
