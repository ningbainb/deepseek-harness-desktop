...
`desktop-v3.1.0` 带来三大核心突破——**安全性与极致稳定性**、**开放丰富的扩展生态**与**实时计费监控**：[查看完整发布说明](docs/launch/release-notes.md) · [查看兼容性和运行时政策](docs/compatibility-policy.md) · [查看升级与回滚](docs/upgrade-and-rollback.md)。发布资产包含 `SHA256SUMS.txt`、`release-manifest.json` 与频道元数据；签名状态以同一 Release 的 manifest 为准。

| 版本 | 主要更新 |
| --- | --- |
| **3.1.0** | **三大核心强化**：① **实时计费与 Token 统计**：新增实时生成吞吐量（tokens/sec）监控，支持官方阶梯计费（含 Cache Read、输入/输出 Token）与夜间闲时优惠自动识别切换；② **全链路防卡死与自愈**：5 轮自适应退避重试与自动换端口自愈彻底解决 100% 进度条停滞，Windows 文件锁（`EBUSY`）容错与坏账 Profile 自动重建彻底消灭 88% 闪退死循环；③ **前端插件安全沙箱（SafePluginBoundary）**：React 错误边界全面隔离第三方插件，插件异常绝不波及主界面，核心按钮与发消息 100% 保持可用；④ **开放生态与精准隔离**：仅外科手术式停用单一故障插件，其余所有第三方插件与历史会话正常保留；⑤ **侧边栏与设置排版完全恢复**。 |
...
- **全链路启动防卡死与自愈（彻底告别 100% 与 88% 停滞）**：
  - **100% 停滞自愈**：渲染器连接增加 5 轮自适应阶梯退避重试（500ms~2000ms），握手失败自动重启服务并换用全新端口，彻底杜绝启动卡死。
  - **88% 闪退熔断重建**：将 Windows 文件锁冲突（`EBUSY`）纳入保护体系，基线 Profile 异常时毫秒级自动备份坏账并从官方基线干净重建，不再反复闪退。
- **前端插件安全沙箱（SafePluginBoundary）**：全面上线 React ErrorBoundary 错误隔离。第三方插件异常仅限制在自身挂载点，主界面的输入框、侧边栏、发消息、设置按钮等 100% 保持交互可用，彻底解决进系统后按钮点不动的假死难题。
- **开放外来插件生态与精准单插件隔离（Stage 1）**：坚持开放生态，不限制外来插件安装；遇到故障时采用外科手术式精准隔离，仅自动停用发生致命错误的单个插件，用户安装的其他所有第三方插件正常保留与加载，本地资产与历史会话 100% 完整持久化。
- **实时 Token 统计与计费看板**：新增实时生成吞吐量（tokens/sec）监控，深度适配官方阶梯计费（含 Cache Read、输入/输出 Token）与夜间优惠/闲时费率自动识别切换。
- **侧边栏与设置排版完全恢复**：彻底清理了全局样式冲突与行内副作用，侧栏在展开/收起状态下均能完美对齐。
- **无凭据零泄露安全回退**：未配置 API Key 时 100% 在本地执行安全回退，毫秒级跳过 AI 修复并进入基线，不发出任何无效外部网络请求。
...
3.0.9 使用 DSH RC.1 内置的 `llm-pi-ai/openai-codex` 官方授权流程，不再加载会与原生 Provider 冲突的旧 `dsh-codex-connect` 插件。设置页的「ChatGPT 登录」一次点击启动 OAuth，并由系统浏览器继续；授权 grant 只由官方凭据服务读写并保存在本机 DSH Home，前端只读取“是否已登录”，不会收到 access token 或 refresh token。它不会默认替换当前模型、接管全局搜索或启用远程图片工具。
