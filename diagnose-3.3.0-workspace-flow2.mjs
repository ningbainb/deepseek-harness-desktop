...
const temporary = await mkdtemp(join(tmpdir(), 'dsh-3-3-workspace-flow2-'))
const userData = join(temporary, 'user-data')
const dshHome = join(temporary, 'dsh-home')
const workspace = join(temporary, 'workspace')
let app

async function runtimeWindow() {
...
async function dismissStartup(page) {
  for (let attempt = 0; attempt < 32; attempt += 1) {
    ...
    if (await star.getAttribute('data-open').catch(() => null) === 'true') {
...
try {
  ...
  const page = await runtimeWindow()
  ...
  console.log(`picker path: ${await input.inputValue()}`)
